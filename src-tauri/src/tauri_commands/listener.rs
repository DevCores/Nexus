        ...
        if let Some(mut test) = vosk::recognize(&frame_buffer, false) {
            if !test.is_empty() {
                    ...
                    assistant_commands::fetch_command(&test, &COMMANDS)
                {
                    ...
                    //ответ в случае неизвестной команды
                    events::play("not_found", &app_handle);
                }
            }
        }
...
pub fn vosk_listen<'s, S, K>(app_handle: &tauri::AppHandle, start_callback: S, mut keyword_callback: K) -> Result<bool, String>
    ...
    // vars
    let fetch_phrase = "нексус".chars().collect::<Vec<_>>();
...
pub fn picovoice_listen<'s, S, K>(app_handle: &tauri::AppHandle, start_callback: S, mut keyword_callback: K) -> Result<bool, String>
    ...
    match PorcupineBuilder::new_with_keyword_paths(picovoice_api_key, &[Path::new(config::KEYWORDS_PATH).join("nexus_windows.ppn")])
